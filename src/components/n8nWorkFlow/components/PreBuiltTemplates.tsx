...
/**
 * Check if a parameter should be excluded as internal workflow logic
 * UNIVERSAL version that works for ALL workflows
 */
const shouldExcludeParameter = (fieldName: string, nodeType: string, value?: string): boolean => {
  ...
  // FIRST: Check if this is a USER-CONFIGURABLE parameter (HIGHEST PRIORITY)
  ...
  // SECOND: Check if this is a UNIVERSAL internal parameter
  ...
  // THIRD: Check for internal logic patterns in the VALUE
  ...
  // FOURTH: Special cases based on node type
  if (lowerNodeType.includes('webhook')) {
    // For webhook nodes, only show user payload fields, not technical config
  ...
  if (lowerNodeType.includes('http') || lowerNodeType.includes('api')) {
    // For HTTP nodes, only show user payload, not technical config
  ...
  if (lowerNodeType.includes('email') || lowerNodeType.includes('smtp')) {
    // For email nodes, show to/from but not pre-defined content
    const emailInternalFields = ['subject', 'message', 'body', 'html', 'text'];
    if (emailInternalFields.some(field => lowerField.includes(field))) {
      // Check if the value looks like a template vs user input
      if (value && (
        value.includes('{{') || 
        value.includes('Alert') ||
        value.includes('Notification') ||
        value.length > 100 // Long pre-defined content
      )) {
        return true;
      }
    }
  ...
  // If we get here, it's not clearly internal or user-configurable
  // Use whitelist approach for safety
...
/**
 * Check if a field name looks like a user input field
 */
...
/**
 * Scan parameters for n8n expression references like {{ $json.fieldName }}
 */
...
/**
 * Determine input type from field name
 */
...
/**
 * SMART parameter extraction that understands workflow context
 */
const extractUserParametersFromObject = (
  ...
  Object.keys(obj).forEach(key => {
    ...
    if (typeof value === 'string') {
      ...
      // Only extract if it passed all the filters above
    ...
    // FOURTH: Recursively process nested objects
...
/**
 * Enhanced function to extract ONLY user-required inputs from workflow
 * Now with UNIVERSAL filtering for all workflows
 */
const extractUserInputsFromWorkflow = (templateJson: string): WorkflowInput[] => {
  try {
    ...
    // Filter out sticky notes and other non-executable nodes FIRST
    const executableNodes = workflow.nodes.filter((node: WorkflowNode) => {
      const shouldExclude = shouldExcludeNode(node.type);
      if (shouldExclude) {
      }
    ...
    if (hasWebhook) {
      ...
      expressionReferences.forEach(fieldName => {
        // Only include webhook references that are NOT internal workflow parameters
    ...
    // Process only executable nodes for credentials and parameters
...
/**
 * Enhanced function to extract integrations - also filters out non-executable nodes
 */
const extractIntegrationsFromWorkflow = (templateJson: string): string[] => {
  try {
    ...
    if (workflow.nodes) {
      // Filter out non-executable nodes first
      const executableNodes = workflow.nodes.filter((node: WorkflowNode) => 
...
const handleCredentialsSubmit = async (
  ...
  try {
    ...
    // FIXED: Properly format the payload for backend
    const payload: any = {
    ...
    // FIXED: Send local time string instead of Date object
    if (executionMode === "scheduled" && scheduledTime instanceof Date && !isNaN(scheduledTime.getTime())) {
      // Convert to local time string in format: "YYYY-MM-DDTHH:mm:ss"
      ...
      console.log('📅 Time conversion:', {
        ...
        toISOString: scheduledTime.toISOString(), // For comparison
    ...
    if (response.ok) {
      ...
      if (result.status === "executing" && result.executionId) {
        ...
        alert(
          ...
          `🆔 n8n ID: ${result.n8nWorkflowId}\n` +
          `⏰ Status: Scheduled\n\n` +
          `The workflow has been scheduled and will run at the specified time.\n` +
        ...
        alert(
          ...
          `📋 Workflow: ${result.workflowName}\n` +
          `🆔 n8n ID: ${result.n8nWorkflowId}\n` +
          `🔗 Status: Activated for external triggers\n\n` +
          `The workflow is now active and waiting for external triggers.\n` +
          `📊 Monitor progress in the n8n dashboard at http://localhost:5678`
        );
